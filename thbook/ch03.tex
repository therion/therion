...
\comopt 
  ...
  * |symbol-set <symbol-set>| = use |symbol-set| for all map symbols,
    ...
    SKBB (Speleoklub Bansk\'a Bystrica)\par
    JSSJ (map symbols from the book {\it Jaskynn\'y syst\'em 
                                      Stratenskej jaskyne})
  * |symbol-assign <point/line/area> <symbol-type> <symbol-set>| = display a
    particular symbol in the given symbol-set. This option overrides 
    |symbol-set| option.
    
    If the symbol has a subtype, |<symbol-type>| argument has form
    |type:subtype|.
  ...
  * |symbol-hide <point/line/area> <symbol-type>| = don't display particular 
    symbol (M, A)
...
Usually this is done from the command line in the data directory as follows
