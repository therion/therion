\eject
{\baselineskip=12pt
\setbox1=\hbox{\eightit We both step and do not step in the same rivers. 
We are and are not.}
\line{\pdfximage {pic/herakl.pdf}\pdfrefximage\pdflastximage\hss}
\copy1
\vskip6pt
\hbox to \wd1{\hfil\eightrm ---Heraclitus of Ephesus, 
             {\eightmit 6}th/{\eightmit 5}th century BC}
}
...
Specifies output coordinate system.

\subsubchapter `sketch-warp'.

Specifies which scrap warping (morphing) algorithm to use. 
Possible switches are |line| - the default, |plaquette| - invented by
Marco Corvi.
...
\arguments
*  |<language ID>| = standard ISO language identifier (e.g. |en| or |en_UK|)
...
\comopt 
  ...
  * |grid-coords <off/border/all>| = where to label grid with
  ...
  * |colo[u]r <item> <colour>| = customize colour for special map
    ...
    For |map-fg|, you can use |altitude|, |scrap| or |map| as color too. 
    ...
    with same colour as associated scrap.
  ...
  * survey-level <N/all> = how many surveys to display with stations.
...
\options
  ...
  * |format/fmt <format>| = set map format. Currently |pdf|, |svg|, 
    ...
    directory with the specified filename.], |kml| (Google Earth) and 
  ...
  * |format/fmt <format>| = set continuation output format. Currently the following
    output formats are supported: |text| (default), |html| and |dbf|.
  ...
  * |location <on/(off)>| = set whether to export location of cave entrances in
    the table.
...
map  & pdf, svg, xhtml, dxf, esri, survex, xvi, kml\cr
atlas & pdf\cr
...
* |-x| =
  Generate file `.xth-thconfig' with additional information for XTherion.
\endoptions
