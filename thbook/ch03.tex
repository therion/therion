...
\fi
  ...
  * |colo[u]r <item> <colour>| = customize colour for special map
    ...
    The maximum ink coverage ($C+M+Y+K$) may be around 240 or 300,
...
* |--reproducible-output| =
        Produce reproducible output PDF and SVG files.
        No volatile information is included (e.g. creation date or software
        version).
        
* |--generate-output-crc| =
        Generate `.crc` file with CRC32 checksum for each output file.
        With reproducible output, calculate and save checksum file.

* |--verify-output-crc| =
        Verify if output file was not changed.
