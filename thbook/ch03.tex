...
\subsubchapter `preview-deep'.

\description
  Show map previews on all previous levels (shifts). If turned off, only show
  the preview on the preceeding level.
\enddescription
...
\options
  ...
  * |map-level <number>| = valid only when a map is selected. Determines
    ...
    expansion is done up to the basic maps (maps of scraps).
