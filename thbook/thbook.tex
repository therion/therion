...
\newdimen\mgshift  % used to adjust horizontal position, required for tables (0 by default)
\def\NEW#1{\iffulloutput\ifnum\versiontonum{#1}<\versiontonum{5.5}% don't display too old news
       ...
       \leavevmode\vadjust{%
	    ...
	    \vss
	    \hbox to 0pt{%
	      \kern\hsize\kern\mgshift
	      \raise2.3pt\rlap{\quad\grey\tenit #1\POPcolor}\hss%
	    }%
	  }%
	}\fi\fi\ignorespaces
      }%
...
\parindent=0pt
\baselineskip=16pt
\setbox\strutbox=\hbox{\vrule height11.33pt depth 4.67pt width0pt}%
\raggedbottom
\widowpenalty10000
\clubpenalty10000
