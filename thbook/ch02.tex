\eject
\vbox to0pt{\line{\pdfximage width 93.2mm {pic/agrippa.jpg}\pdfrefximage\pdflastximage\hss}\vss}\kern-2pt
...
\description
 ...
 % Each data object must belong to a   survey. 
  ...
  to be built. Usually at some level of this hierarchical structure
  survey represents a cave. 
...
* |entrance <station-name>| = specifies the main entrance to the cave represented
  ...
  in this survey, it is considered to represent a cave also.
\endoptions
...
\comopt
  ...
  * |flags <shot flags>| = set flags for following shots. The supported
    ...
    in extended elevation by default). Those are excluded from length 
    calculations. 
    
    If flag is set to |approx[imate]|, it is included to total length
    calculations, but also calculated separately in survey statistics.
  ...
  * |station <station> <comment> [<flags>]| = set the station comment
    ...
    Supported flags: |entrance|, |continuation|, |sink|, |spring|, |doline|,
    |dig|, |air-draught[:winter/summer]|. Also ``|not|'' is allowed before a flag, 
    to remove previously added flag.
    
    You can also specify custem attributes to the station using |attr| flag
    ...
    |continuation| flag. Just add |explored <explored-length>| to the station 
...
\options
  ...
  * |explored <length>| = if the point type is continuation, you can specify
    ...
    used in survey/cave statistics.
...
* in maps generated using \MP\ (PDF, SVG) the attributes are written in 
  the \MP\ source file (in the form |ATTR_<name>|) and can be evaluated and used by user in 
  symbols definition macros.
  
  If |<value>| is a number, then the type of |ATTR_<name>| variable in \MP\ is 
  numeric, otherwise it is string. You can test presence of such a 
  variable using |if known ATTR_<name>: ... fi|, or to check the type by using
  |if numeric ATTR_<name>: ... else: ... fi|. 
  
  Numeric variable can be converted
  to string using |decimal ATTR_<name>|.
