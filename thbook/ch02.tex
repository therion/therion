...
\NEW{5.3}|<lang:XX>| = in multilingual labels only the text between |<lang:XX>|
    (where |XX| is input language selected in thconfig) and the next |<lang:YY>| 
    tag is displayed. If no match is found, everything before any occurence of
...
on georeferencig data included in a PDF map\[Present if centreline contains
...
on your system. Note, that Windows instalation does not include |ghostscript|] 
to a raster format.
