...
|layout| and then enter XY data in given system. 
You can also specify output |cs| 
in configuration file. If you do not specify any |cs| in your dataset, 
it is assumed you are working in |local| coordinate system, and no 
conversions are done. Following coordinate systems are supported:
...
* |EPSG:<number>| = Most of EPSG coordinate systems. Almost every 
  coordinate system used worlwide has its own EPSG number. To find
  number of your system, see |extern/proj4/nad/epsg| file in the
  therion source distrubution.
* |ESRI:<number>| = Simmilar to EPSG, but ESRI standard.
...
Therion contains built-in IGRF Earth geomagnetic field model valid for period 
1900--2010. It is automatically used if the cave is located in the space using 
...
\comopt
  ...
  * |team <person> [<roles>]| = a survey team member. The first argument
    ...
    gradient, counter, depth, station, position, notes, pictures, pics,
  ...
  * |flags <shot flags>| = set flags for following shots. The supported
    ...
    duplicated surveys). Both are excluded from length calculations.
  ...
  * |station <station> <comment> [<flags>]| = set the station comment
    ...
    comment, it is ignored. If station has continuation flag set, you
    can also specify arbitrary continuation code using code flag followed
    by code string e.g.\hfil\break
    |station 4 "pit to explore" continuation code V|
  ...
  * |walls <auto/on/off>| = turn on/off passage shape genaration from
    LRUD data for subsequent shots. If set |auto|, passage is generated
    only if there is no scrap referencing given centreline.
  * |vthreshold <number> <units>| = treshold for interpreting LRUD readings
  ...
  * |extend <spec> [<station> [<station>]]| =
    ...
    |left/right| = same as above, but direction is specified explicitely.

    |vertical| = no not move station (shot) in X direction, use only Z
...
\options
  ...
  * |cs <coordinate system>| = coordinate system for scrap scale real points
    points. If CS is specified, no station is needed in scrap - it is placed according
    to CS specification.
...
\arguments
  ...
  * |<type>| determines the type of an object. The following
   ...
   (which is 0 by default).
    ...
    |gradient|, |map-connection|\[Virtual point, used to indicate connection
    between shifted maps.], |extra|\[Additional morphing point.], 
...
\options
  * |subtype <keyword>| = determines the object's subtype. The following
   ...
   if it's specified there] 
    ...
    `|:|' as a separator.\[E.g.~|station:fixed|] Any subtype specification can
    be used with user defined type (|u|).
  ...
  * |dist <distance>| =  valid for extra points, specifies distance to nearest
  ...
  * |code| = arbitrary extra code used when continuation is marked.
    It is usually used to express quality of continuation.
...
\arguments
   * |<type>| is a keyword that determines the type of line.
     ...
     or same points in extended elevation).], |u|\[For user defined line symbols.].
\endarguments
...
\comopt
    ...
    The subtype may be specified also directly in |<type>| specification using 
    `|:|' as a separator.\[E.g.~|border:invisible|] Any subtype specification can
    be used with user defined type (|u|).
  ...
  * |adjust <horizontal/vertical>| = shifts the line point to be aligned
    horizontaly/ver\-ti\-caly with the previous point (or next point if there is 
...
\description
Surface (terrain) specification. It's possible to display it in two ways: as a 
scanned topographical map (currently in 2D maps only) or surface grid 
(3D model).
\enddescription
...
alphanumeric characters, |<value>| may be either a number or a string. 
...
the \MP\ source file (in th form |ATTR_<name>|) and can be used by user in symbols definition 
macros.
\endlist
...
displays centreline and LRUD information on the background.
...
* move pointer to desired position, read pointer coordinates from 
...
X2,Y2 coordinate pairs the calibration arrow wil be moved correspondingly. 
\endlist
