...
\options
  ...
  * |id <ext_keyword>| = ID of the symbol.
   ...
   * |context <point/line/area> <symbol-type>| = (to be used with |symbol-hide|
    ...
    |-context point air-draught| to a label which displays the 
    observation date, the |symbol-hide point air-draught| command would hide 
    both air-draught arrow and the corresponding label.] Only supported on point 
    types air-draught, height, passage-height, station and water-flow.
...
\comopt
  ...
  * |visibility <on/off>| = displays/hides the symbol.
   ...
   (which is 0 by default).
       ...
       * |context <point/line/area> <symbol-type>| = (to be used with |symbol-hide|
         and |symbol-show| layout options) symbol will be hidden/shown according
         to rules for specified |<symbol-type>|. Only supported on line types 
         border, survey, wall and water-flow.
