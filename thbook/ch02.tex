...
\options
  ...
  * |flip (none)/horizontal/vertical| = flips the scrap after
    scale transformation. Default is |none|.
  * |station-names <prefix> <suffix>| = adds given prefix/suffix to
...
\options
  ...
  * |scale| = symbol scale, can be:
    ...
    $xl \equiv 2.0$.
  * |place <bottom/default/top>| = changes displaying order in the map. Default is 
    |default|.
  * |clip <on/off>| = specify whether a symbol is clipped by the scrap border.
    ...
    station-name, label, remark, date, altitude, height, passage-height. Default 
    is |on|.
  * |visibility <on/off>| = displays/hides the symbol. Default is |on|.
...
\comopt
    ...
    (see the chapter {\it New map symbols}).
       ...
       * |adjust <horizontal/vertical>| = shifts the line point to be aligned
         ...
         This option is not allowed in the |plan| projection. Default is unset where
         the line point isn't automatically aligned with the previous line point.
   ...
   (which is 0 by default).
       ...
       * |gradient <none/center/point>| = can be used only with the contour
         ...
         option must be used inside |[LINE DATA]|. The others can (and should)
         be used as a line option. Default is |none|.
       * |head <begin/end/both/none>| = can be used only with the arrow
         type and indicates where to put an arrow head. Default is |end|.
