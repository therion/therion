...
Installing dependencies to compile Therion on {\bf Ubuntu 22.04}:
...
Installing dependencies on {\bf Ubuntu 24.10}:
...
Installing dependencies in {\bf termux} on {\bf Android}:

{\rightskip 0pt plus 3cm
  |pkg install|
  |clang cmake fmt ninja perl proj tcl texlive-bin|
\par}
Then run |termux-install-tl| to install additional texlive packages and
|termux-setup-storage| to make Android data directories accessible from termux.
...
{\it Compilation on Android}\NEW{6.3}

Install the dependencies as listed above and run the following in the therion source directory:

\list
* |mkdir build && cd build|
* {\catcode`\=12|cmake -DUSE_BUNDLED_CATCH2=ON -DBUILD_LOCH=OFF -G Ninja ..|}
* |ninja|
\endlist
...
modifying the files in the |src/therion-mpost| and |src/therion-tex| directories it's necessary to run
Therion executable compilation again, which will also generate C++ header files.
...
*  scraps count =
   ...
   are used to store references to the graphic objects.]
            ...
            set |pdftex-path  "<path-to-luatex>"| in the initialization file.])
   ...
   the initialization file.] limited only by memory available to \TeX
*  TeX memory =

   pdf\TeX: static memory limits are set in the |texmf.cnf| configuration file in your \TeX\ distribution;
   note that the total memory available is quite limited, regardless of the RAM available

   Lua\TeX: the memory is dynamically allocated
\endlist
...
* {\bf 2025}\Nobreak
  ...
  May 8: experimental support for 3D meshes.
\endlist
