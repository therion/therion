...
* GNU make
* Perl
* Tcl/Tk 8.4.3 and newer (|http://www.tcl.tk|) with 
  {\it BWidget} widget set (\path|http://sourceforge.net/projects/tcllib|)
  and optionally 
  {\it Tom} OpenGL extension (improved version is included in Therion source 
  distribution)
  and {\it tkImg} extension (\path|http://sourceforge.net/projects/tkimg|). 
  
  
  
* \TeX\ distribution with at least \TeX\ with Plain format, 
  recent pdf\TeX, and \MP\ (\path|http://www.tug.org|). 
\endlist

All programs (with the exception of BWidget, Tom and tkImg package) are usually 
included in Linux, Unix or MacOS\,X distributions.
For Windows consider using MinGW and MSYS (|http://www.mingw.org|).
...
\list
* unpack source distribution |therion-0.3.*.tar.gz|
* |cd therion|
...
* |language <xx[_YY]>|  
  ...
  Default output language (used for maps). Currently supported languages are
  |sk| (Slovak), |en| (English) and |cz| (Czech).
...
* map rotation
* map grid
* complete map symbol sets
* area blocks
* SVG support
* centreline without scraps in maps
* cross-sections in the margin/appendix
\endlist
