...
Installing dependencies to compile Therion on Ubuntu 18.04 or 20.04:
  |sudo apt install|
  |bwidget|
  |cmake|
  |gcc|
...
Installing dependencies in Fedora (tested on Fedora 37):
...
Other necessary pre-compile configurations:
\list
* {\catcode`\=12|sudo ln -s libnghttp2.so.14 /lib64/libnghttp2.so|}
* {\catcode`\=12|sudo ldconfig|}
* {\catcode`\=12|export CMAKE_C_COMPILER=gcc|}
\endlist
