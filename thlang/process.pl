#!/usr/bin/perl
## usage: ./process.pl texts.txt->.cxx,.h
##        ./process.pl update - updatuje definicie a nacita vsetky texts_xy.txt
##                             subory
##        ./process.pl export xy - exportuje texts_xy.txt subor
...
sub export_language {
  (my $fn, my $lng, my $href) = (shift, shift, shift);
  my %hr = %{$href};
  open(OUT,">$fn") || die("error: can't open $fn for output\n");
...
if ($ARGV[0] =~ /^update$/i) {
  ...
  export_language("texts_$lng.txt",$lng,$rf);
} else {
  write_sources($rf);
}
