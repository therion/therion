...
\def\setsize[#1]{\size[#1]\basefontsize=#1\relax }

\def\TITLE#1{\eject\hbox{}\vskip5cm\centerline{\setsize[24]#1}\vfil\eject}
...
\def\dopage{%
 \vbox{\centerline{\framed{\mapbox}}
  ...
  \line{%
    \vbox to \ht\navbox{
      \hbox{\setsize[20]\the\pagelabel
        \ifpagenumbering\space(\the\pagenum)\fi
        \space\setsize[16]\the\pagename}
      \ifpagenumbering
        \medskip
        \hbox{\qquad\qquad
          \vtop{%
...
\newtoks\legendtextsize\legendtextsize={\setsize[12]}
\newtoks\legendtextheadersize\legendtextheadersize={\setsize[26]}
\newtoks\legendtextsectionsize\legendtextsectionsize={\setsize[14]}
...
\def\atlastitlepages{
  ...
  {\rightskip=0pt plus 3em\parskip=3bp
    \edef\tmp{\the\comment} \ifx\tmp\empty \else
      {\setsize[12]\the\comment\par}\medskip
    \fi
    \everypar{\hangindent=2em\hangafter=1}
    \edef\tmp{\the\exploteam} \ifx\tmp\empty \else
      {\setsize[12]\si\the\explotitle: \ss\the\exploteam\par}
    \fi
    \edef\tmp{\the\topoteam} \ifx\tmp\empty \else
      {\setsize[12]\si\the\topotitle: \ss\the\topoteam\par}
    \fi
    \edef\tmp{\the\cartoteam} \ifx\tmp\empty \else
      {\setsize[12]\si\the\cartotitle: \ss\the\cartoteam\par}
    \fi
    \edef\tmp{\the\copyrights} \ifx\tmp\empty \else
      {\setsize[12]\ss\the\copyrights\par}
    \fi
    \edef\tmp{\the\cavelength} \ifx\tmp\empty \else
      {\the\legendtextsize\si\the\cavelengthtitle: \ss\the\cavelength\par}
    \fi
...
%% end of eplain macros

% use \twodigit\day instead of \the\day to get a leading zero
