...
void thdb3ddata::export_tlx(FILE * out) {
  thdb3dfc * fc;
  thdb3dfx * fx, * fx1, * fx2, * fx3;
  thdb3dvx * vx;
  long id1;
  id1 = 0;
  for(vx = this->firstvx; vx != NULL; vx = vx->next)
    fprintf(out,"P %ld %8.3f %8.3f %8.3f\n", id1++,
      vx->x - this->exp_shift_x, 
      vx->y - this->exp_shift_y, 
      vx->z - this->exp_shift_z);
  ...
  for (fc = this->firstfc; fc != NULL; fc = fc->next) {
    switch (fc->type) {
      ...
      case THDB3DFC_TRIANGLE_STRIP:
        ...
        if (id1 > 2) {
          fprintf(out,"L 2 %ld", id1);
          fx = fc->firstfx;
          while (fx != NULL) {
            fprintf(out," %ld", fx->vertex->id);
            fx = fx->next;
          }
          fprintf(out,"\n");
        }
